...
// renders the systemd .mount units that topology needs and the argv a
// caller execs to bring one up directly (the loop-device lab has no
// init system, doc 06 §3). It computes no placement of its own: which
// disk a file lands on is mergerfs's create policy, never this
// package's (doc 09 §2, "one placement algorithm").
package pool
