...
// A sqldef limitation Q60 records: sqldef's ADD COLUMN generator silently
// drops a new column's own REFERENCES clause — `ALTER TABLE a ADD COLUMN
// parent_id integer` with no `REFERENCES parent(id)` at all, confirmed
// directly against schema.GenerateIdempotentDDLs. Unlike DEFAULT, CHECK and
// COLLATE (all of which the generator does carry through on ADD COLUMN, per
// the three tests above), this is not a case the generate-then-check
// workflow can complete silently and correctly. `run` itself runs the
// same drift check `make db-check` would, on its own candidate output,
// before writing anything — so this case is refused immediately, and
// never gets to write or checksum a migration that would already fail
// db-check.
func TestRun_RefusesGeneratedMigrationThatWouldNotReproduceSchema(t *testing.T) {
	...
	if err := run("next", dir); err == nil {
		t.Fatal("expected run to refuse a migration that would not reproduce schema.sql (sqldef silently drops the new column's REFERENCES clause)")
	}
