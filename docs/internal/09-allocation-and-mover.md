...
Other operations treat a disk in removal as leaving. Rebalance leaves it out entirely (§3): it moves nothing off the disk and nothing onto it. The mover's room pre-check (§2) does not count its free space, since mergerfs creates nothing on it. Share relocation leaves it out too, except while it is `evacuating`, when a relocation to cache still has to read that share's files from it. Upgrading it is always refused (`disk_leaving_array`), because the new disk would inherit its removal state. Replacing it is refused the same way while it is `evacuating` or `unlisted` — an evacuation still running is not replace's job, and a disk already dropped from `snapraid.conf` has nothing left for `snapraid fix` to rebuild against — but not while it is `evacuated` or `unpooled`: the evacuation has already succeeded, and the disk can since have died with SnapRAID still recording a file its own post-check never inspected (a file outside every share, above). Rebuilding that file onto a replacement is the only way to keep parity's last copy of it, so replace is allowed there once the old disk is genuinely missing — nothing is mounted at its slot and nothing in a fresh inventory still carries its identity, the same check that already refuses a replace of a disk that has not actually failed (doc 02 §4 "Replacing a failed disk"). The replace job abandons the removal in the same store write that adopts the replacement's identity, so the slot rejoins the pool as an ordinary disk, `snapraid fix` reconstructs its recorded files, and every pool mount is regenerated with it create-eligible again. Evacuating a disk again is allowed while it is `evacuating` or `evacuated`, to resume or repeat the evacuation; once it is `unpooled` or `unlisted`, only finishing the removal or (for `unpooled`) a replace takes it further.
