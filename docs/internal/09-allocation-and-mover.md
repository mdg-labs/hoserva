...
2. Put the disk in `removing` state: set its branches to no-create (`NC`) in every mount, so nothing new lands on it; the zero-files guard rule is suspended for this disk only (Q15)
