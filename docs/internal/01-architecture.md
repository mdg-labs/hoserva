...
2. Before the first pending migration, it writes a consistent snapshot with `sqlite-migrate`'s own `Snapshot` to `/var/lib/hoserva/backups/pre-migration/` (doc 10 §1's consistency rule), keeping the snapshots of the last three upgrades — the library's own `SnapshotDir`/`RetainSnapshots` fields, not bespoke snapshot code.
