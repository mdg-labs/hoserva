...
6. **Confirm and create** — explicit list of which disks will be erased, requiring typed confirmation. Then a job with progress. Once that job succeeds, the wizard calls `array start` (idempotent if the array is already up) before leaving for the Storage overview — array creation itself only persists topology, mounts each physical disk and rebuilds the daemon's own stop/start sequence; it does not bring the pool up on its own, and no daemon restart is needed at any point in this flow (#262).
