...
The image also carries `samba` and `smbclient` (issue #219), so `make test-integration` can run a real `smbd` against a Hoserva-generated `smb.conf` — this is doc 12 §3's `test-integration` target, previously unimplemented. `scripts/devenv/gen-smb-conf` renders that config on the host, from `internal/config.RenderSambaConf`'s own output for a single test share (the lab image has no Go toolchain, unlike the native `snapraid`/`mergerfs` packages), and `scripts/devenv/smb-check.sh` then does everything Samba-related inside the container: it starts `smbd`, writes a file over SMB as uid:gid `99:100` (Q26) and confirms a plain filesystem read as that identity sees the same content, then does the same check in the other direction (a file written directly on the filesystem as `99:100`, read back over SMB) — the concrete proxy for #48's own "files written over SMB and by a container as `99:100` are mutually readable and writable" acceptance criterion. RenderSambaConf now emits #48's own Q26 masks and force modes on every share (`create mask = 0664` with `force create mode = 0664`, `directory mask = 2775` with `force directory mode = 2775`, and `force group = users`), so the check exercises Samba's behaviour with those directives applied; the assertions themselves only compare uid:gid ownership and content, not exact mode bits, so they hold regardless. CI runs it on every merge-gate lab job (§7), alongside `lab-verify-refusal` and `lab-snapraid-check`.
...
`scripts/devenv/destroy-array.sh` runs **from inside the container**, because everything the lab writes under `$LAB` is owned by root. In order: unmount every mount *strictly below* `$LAB` deepest-first (via `findmnt --submounts`, so a per-share mergerfs mount nested inside the catch-all goes before the thing it sits on) — `$LAB` itself is excluded from this loop, because it is the container's own bind mount to the host's `.lab/<id>`, not an array mount to tear down; detach every loop device whose backing path is under this lab, read from `/sys/block/loop*/loop/backing_file`; assert `$LAB` is still a mountpoint, `die`ing otherwise; then delete the contents of `$LAB`. Only then is the container removed, and `make lab-destroy`'s final host-side step is an `rmdir` of an already-empty directory — never a recursive delete that could not have succeeded.

The mountpoint assertion exists because the failure mode is a check that silently passes on the wrong directory: if `$LAB`'s bind mount is ever gone by the time teardown reaches the delete step, `rm -rf` would run against the container's own empty overlay directory instead, "succeed" on nothing, and strand every root-owned file under the host's `.lab/<id>` with no way left to remove them (issue #122). Teardown must fail loudly in that state, never report success.
...
|---|---|---|
| **Spindown** (Q31) | In the lab, with realistic idle/appdata/SMB-client load: per-disk read and write counters (`/sys/block/<dev>/stat`) stay flat for 30+ minutes of the Q31 scenario, with IO attributed to a process (fanotify) confirmed as a working measurement tool (doc 08 Spike 1). In L3, `scripts/vm/spindown-check.sh` (`make vm-suite` steps 9-10) confirms the same flat-counter property over the same 30-minute window while looping the SMART poller's own command (doc 08 Spike 1's L3 addendum, issue #33) | A drive's firmware or controller waking it with no host IO; `hoservad` has no scheduled SMART-poll or change-journal job wired yet, and no array/pool operation exists in `api/openapi.yaml` for an end-to-end run against a real Hoserva-managed pool — so neither tier has yet measured this with SMART polling and the change journal *actually* running, only with the SMART poller's command looped directly (doc 08 Spike 1's L3 addendum) |
| **SMART polling without waking disks** | Real `smartctl -j` output from many drive models as parser fixtures (L1); in L3, `spindown-check.sh` confirms the poller's exact `-n standby` argv causes zero `/sys/block/<dev>/stat` movement on every array disk over a sustained window (doc 08 Spike 1's L3 addendum, issue #33) | Firmware that spins up on a SMART query despite `-n standby` — and, specific to this harness, unprovable at all: a virtio-blk-attached qcow2 disk returns no real SMART telemetry (`smartctl` exits 1, "Unable to detect device type"), so L3 confirms "no IO reaches the disk" but never "a real disk correctly honored standby" — only real hardware could, and D20 rules that out |
| **Disk identity** (Q21) | L3 virtual disks, each with a configured `<serial>` that `internal/disk.ResolveIdentity` resolves via the guest's own `virtio-<serial>` `/dev/disk/by-id` link (confirmed against a real L3 guest through the real, unmodified `disk.LinuxProvider.List()` path, issue #162), and USB-attached virtual disks with the serial hidden | Enclosures and HBAs that report identity inconsistently. libvirt itself does not support a `<wwn>` on a virtio-blk disk ("Only ide and scsi disk support wwn", confirmed against this project's libvirt), so an L3 array disk is identified by its `virtio-` by-id link rather than a `wwn-` one the way a real SATA/SAS drive would be — `create-vm.sh` leads each array disk's `<serial>` with its own disk role name (not `$HOSERVA_LAB_ID`) specifically so the guest kernel's `VIRTIO_BLK_ID_BYTES` truncation (20 bytes) can never collide two array disks' by-id links regardless of `$HOSERVA_LAB_ID`'s length, confirmed at both a short lab id and a nightly-shaped one (`nightly-<run_id>-<attempt>`). Real hardware is unaffected — ata-/scsi-/nvme-/usb- are exactly what a real SATA/USB/NVMe drive's own by-id links use, and `ResolveIdentity` recognizing `virtio-` as well changes nothing about how those are read |
| **Array stop/start sequence** (doc 02 §1, §4, Q69, Q70, issue #146) | `scripts/vm/array-sequence-check.sh` (`make vm-suite` step 11) builds `internal/job/array_l3_test.go` (`-tags l3`) and runs it inside the VM: a real data disk is detached from the domain's own persistent config and the guest genuinely reboots without it, proving `disk.StorageGate` reports the array not ready and `job.ArraySequence.Start` refuses (`ErrStorageNotReady`) without mounting any of the real physical disks, the real mergerfs catch-all or share mount it built, or starting its (fake) service — and that acknowledging brings the same sequence up for real, including a real systemd-managed physical disk mount/unmount cycle internal/job/array_lab_test.go's own L2 test could not reach. A second scenario proves a real, held-open file genuinely blocks `fusermount -u` on a real mergerfs mount (confirmed as a real hazard, not assumed), and that `ArraySequence.Stop`, run as intended, stops the (proxy) service holding it open before unmounting, so the unmount succeeds and the file survives byte-identical | Everything about a real container runtime's own stop-hook behavior — the "service" is a proxy `tail -f` process standing in for `internal/container`, which does not exist yet (Phase 3, epic #66); and whether a live `hoservad` process reaches this same not-ready→ready transition through its own real boot path and `POST /array/degraded/acknowledge` operation (#372, #385) — both are wired into `cmd/hoservad` now, but this script still constructs `StorageGate`/`ArraySequence` directly rather than driving a live daemon and its real HTTP API, the same way `array_lab_test.go` already does at L2 |
| **Disk yank and reconstruction** (doc 02 §4 "Replacing a failed disk", issue #386) | `scripts/vm/disk-yank-check.sh` (`make vm-suite` step 5): with files of known checksums seeded directly onto a data disk's own mountpoint and a baseline sync run, the guest is shut down and `virsh detach-disk --config` removes that disk from the domain's own persistent config for real — the same detach convention array-sequence-check.sh and storage-target-boot-check.sh already use, never a live hot-unplug. The guest reboots without it, `getPool` is confirmed to report the slot `missing`, a brand-new blank qcow2 image is attached at the exact same `<target dev>`/PCI slot, and `planDiskReplace` then `replaceDisk` drive the real API end to end: the replacement is formatted, mounted back at the same `/mnt/diskN`, and `snapraid fix` reconstructs it. Every seeded file's SHA-256 is confirmed identical afterward, both read from the disk's own mountpoint and served through the pool, and a following `POST /parity/diff` comes back with every group's count at zero | A live hot-unplug — this harness always shuts the guest down first, so a disk that fails mid-write, or firmware/enclosure behaviour a real drive failure shows, is not exercised; SMART-driven failure detection, since `hoservad` has no scheduled SMART-poll job wired yet (see the Spindown row above) — this check always starts from a disk already fully absent, never one the system had to notice failing |
...
**Authentication is a deliberate simplification, not a real check.** For `Login`, any credential accepted returns the mock's one canned admin, already signed in — there is no password check — and no request's session is ever validated afterward: `GetCurrentSession` always reports that same admin, and `Logout`, `EnrollTotp` and `ConfirmTotp` all succeed unconditionally. The #272 contract test (below) does not cover `Login`, `Logout`, `GetCurrentSession`, `EnrollTotp` or `ConfirmTotp` for that reason — see `contractSkip`.

**Fixtures are shared with backend tests**, so the mock cannot drift from reality in *shape*: if the API response shape changes, both break together, and because the mock implements the generated interfaces, a spec change it doesn't follow fails to compile.

Shape parity does not cover *behaviour* — the mock has always had its own validation code, separate from `internal/api`'s, and it has drifted before (unknown notification-channel ids accepted, a topology plan missing a filesystem/cache-disk check, a share `cache_mode` default that disagreed, token-role rules missing entirely, an unregistered external-disk array-member check). `cmd/mockapi`'s own contract test (`TestContract_MockMatchesProductionValidation`, `cmd/mockapi/contract_test.go`, `contract_cases_test.go` and `contract_rig_test.go`, #272) closes that gap for what it covers: one table of requests runs against both `cmd/mockapi`'s handler and an `internal/api.Handler` built from the same fakes `internal/api`'s own package tests use (schedules, settings, UPS, network/ACME/TLS, external disks, host-config/doctor, metrics, wake events, the update engine, sessions and share permissions all included), and the test fails on any difference in HTTP status or error code. `make test` runs it.

What it guarantees is narrower than "every operation matches": for each operation with a table entry, status and error code agree across every case in the table — a validation failure and, where the rig can reach one, at least one request that succeeds on both sides (never response-body content, which the generated interfaces already enforce). Two reviewed, reasoned lists say where that falls short, and why, rather than leaving it unstated or silently loosened: `contractSkip` names operations no case can compare at all — the mock's own request-scoped session model, or a fake no test in the repo builds, means any case would encode the mock's own design choice as a bug, not real drift; `contractNoValidCase` names operations that do have failure cases comparing status and code, but no case that succeeds on both sides. Four are job-run/timing state this rig's no-op job types cannot produce (the issue's own out-of-scope list): an evacuation actually finishing for `FinishDiskRemoval`, a job log file its own goroutine has not yet written for `GetJobLog`, a queued job's cancellable window for `CancelJob`, an interrupted, resumable job for `ResumeJob`. Two more: the mock's own session layer never populates real session state for `RevokeSession` to revoke, and `ImportConfig` needs a fully wired `backup.Service` no fake in the repo builds. The three root-only recovery operations (`ResetUserPassword`, `DisableUserTotp`, `UnlockUser`) refuse every call in the mock unconditionally, by design — the mock carries no peer-credential layer at all, unlike production, which a test can satisfy with a root peer credential (`auth.WithPeerCredential`, `internal/api/recovery_handler_test.go`); this rig's own calls carry none either, so no case here reaches a passing mock response. A new operation with no table entry, and no entry in either list, fails the test until one is added.
