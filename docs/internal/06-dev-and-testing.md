...
| **Disk identity** (Q21) | L3 virtual disks, each with a configured `<serial>`, and USB-attached virtual disks with the serial hidden | Enclosures and HBAs that report identity inconsistently. Also confirmed while building issue #146's L3 test: `create-vm.sh`'s own domain XML sets no `<wwn>` on any array disk, and `internal/disk.ResolveIdentity` does not recognize the `virtio-<serial>` `/dev/disk/by-id` link libvirt/udev create for a virtio-blk disk — it only handles `wwn-`/`ata-`/`scsi-`/`nvme-`/`usb-` prefixes — so `disk.LinuxProvider.List()` reports every L3 array disk with an entirely empty `Identity` today (`WeakIdentity`, no WWN, no Serial), and `disk.Identity.Matches` can never match one against anything. Real hardware is unaffected (ata-/scsi-/nvme-/usb- are exactly what a real SATA/USB/NVMe drive's own by-id links use); this only blocks identity-based L3 testing (e.g. a future add-disk-matching test) against this harness until it's fixed — either the harness configuring a real `<wwn>`, or `ResolveIdentity` recognizing `virtio-` |
