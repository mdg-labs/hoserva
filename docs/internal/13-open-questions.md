...
**Status:** Spike (S9) — **loop devices, FUSE and a hosted SnapRAID sync confirmed on the pinned hosted runner image; `/dev/kvm` now CONFIRMED on a hosted runner too** (issue #10, doc 08 §9 "Hosted CI runners"), so L3's hosted path is now open, pending S10 (nested KVM); **AppArmor necessity still open** — two separate runs' attempts each produced a void result for an unrelated reason, not a real verdict either way · **Gate:** Phase 1 · **Affects:** doc 06 §7, doc 07 R11
...
**Default:** everything that executes pull-request code runs on GitHub-hosted runners: L1, L2 loop devices via the runner's `sudo`, `.deb` build, and L3 if hosted KVM proves sufficient. There are no self-hosted runners (Q79). Workflows from first-time contributors require approval (a repository setting). **What's actually confirmed so far, not assumed:** `ci.yml`'s own `lab` job has run loop devices, XFS and a mergerfs pool mount successfully on a hosted runner in production (run 34950031773, 2026-09-15, on `ubuntu-latest`). The maintainer then triggered `s9-hosted-probe.yml` twice. The first time (run 35049304081, `workflow_dispatch`, 2026-09-16): its `lab` job succeeded **on the pinned `ubuntu-24.04`**, closing that reproducibility gap, and ran a SnapRAID sync to completion on a hosted runner for the first time (`Everything OK`) — L2 plus a real parity sync both work hosted, on the pinned image; its `kvm` job failed at an unreadable-kernel check before `qemu-system-x86_64` was ever invoked (the runner's own real kernel image exists but is `0600 root:root`), though it did confirm the udev-rule mechanism itself works (`crw-rw----` → `crw-rw-rw-`) and that the CPU exposes `svm`. That run's AppArmor with/without comparison was **void, not negative**: both branches failed identically from a probe-recipe bug (missing `mknod` of the container's own loop nodes) unrelated to AppArmor, yet the step still printed a "required" verdict — retracted in doc 08 §9. The kernel-read bug was fixed (copy the running kernel out with the runner's own job-scoped `sudo`, `chown` the copy) and validated locally before the second trigger. **The second run (35056076616, `workflow_dispatch`, `main` at `1388f83`, 2026-09-16) confirmed the KVM fix for real**: the `kvm` job's `## QMP query-kvm reply: {"return": {"enabled": true, "present": true}}` and `PASS: guest booted to its own init in 1s` establish, host-side, that a QEMU guest boots with KVM acceleration on a standard hosted runner. That same run's `lab` job failed at the AppArmor step again, but for two new, narrower, non-AppArmor reasons this time: the probe images (`truncate -s 64M`) were below `mkfs.xfs`'s ~300MB minimum, and the resulting failure left the teardown step's `HOSERVA_LAB_ID` empty (a step-ordering bug). Both are fixed (512M images, the lab id now derived before the AppArmor step); the fixed step has not yet run hosted, so AppArmor necessity remains open. `/dev/kvm` support is no longer the open half of L3's hosted-runner question — S10 (nested KVM, VM-in-VM, a different question) is.
...
**Status:** Default — **validated on the primary dev host** (S9, doc 08) and **SnapRAID's own behaviour on the loop-device lab confirmed, with one named structural gap** (S5, doc 08); hosted CI runners still to check · **Gate:** Phase 1 (foundation) · **Affects:** doc 06 §3, §6, doc 12 §5, `CLAUDE.md`
...
**Status:** Default — **S9's lab half (loop devices, FUSE, a SnapRAID sync) is confirmed hosted on the pinned image, and S9's KVM half is now CONFIRMED hosted too** (run 35056076616, issue #10, doc 08 §9) — a QEMU guest boots with KVM acceleration on a standard `ubuntu-24.04` hosted runner, host-side confirmed via QMP; **S9's AppArmor-necessity question is still open** (two separate runs' attempts each produced a void result for an unrelated, now-fixed reason, not a real verdict); **S10 (nested KVM, VM-in-VM) is a different question and remains entirely untouched, out of scope here** · **Gate:** Phase 1 · **Affects:** doc 06 §4, §7, doc 14 §8, Q42, D20
...
A self-hosted runner is a machine the maintainer owns and exposes to CI, which D20 rules out. A mandatory pre-release agent run keeps every suite required without any new infrastructure. **S9's KVM result now exists and is a pass: a single, non-nested QEMU guest boots with KVM acceleration on a hosted runner.** That opens the hosted path for L3 workloads that need exactly that (a single accelerated guest) but not for anything requiring nested virtualisation (a VM inside the L3 VM) — S10 is a distinct, still-open question, and until it resolves, any suite that needs nested KVM stays on the "agents run it on the dev host before every release" path. Which specific suites this unblocks for the hosted path is a follow-up scoping decision, not settled by this entry alone.
