...
**Decided by the maintainer (2026-09-17):** an `INSERT INTO t SELECT ... ON CONFLICT DO UPDATE` inside a contract step used to classify as `classContractOnly` (its leading shape is `INSERT ... SELECT`), which made it effectively an `UPDATE` on every conflicting row despite the previous paragraph's "never UPDATE" — `classifyInsert` (#134) now refuses any `INSERT ... SELECT` whose statement carries an `ON CONFLICT` clause at paren depth 0 after the `SELECT`, keeping a contract step to a plain copy and pushing any data change into a Go transform instead; an `ON CONFLICT` that can only ever appear as string data nested inside the select itself is not mistaken for one. Separately, an ordinary (non-contract) migration may create a `CREATE TRIGGER` whose body contains DML, which then fires on a later transform's or a contract step's own DML inside the same migration transaction, and again at ordinary runtime; a trigger with a DML body is decided to be **contract-only** (registered), also tracked as its own follow-up `safety-critical` issue and **not built yet** — today's classifier still allows it as an ordinary migration statement.
