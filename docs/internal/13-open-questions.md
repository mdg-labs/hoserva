...
- **A new column's own `REFERENCES` clause is silently dropped** — confirmed directly against `schema.GenerateIdempotentDDLs`: a `schema.sql` change that adds `parent_id INTEGER REFERENCES parent(id)` to an existing table generates `ALTER TABLE `a` ADD COLUMN `parent_id` integer` with the foreign key gone entirely, not an error and not a skipped statement. `DEFAULT`, `CHECK` and `COLLATE` on a *new* column are all carried through correctly (confirmed by the generate-then-drift-check tests in `internal/store/tools/dbmigration/main_test.go`) — this is specific to `REFERENCES`. **Decided (maintainer, 2026-09-17):** the fix is a small Hoserva post-processing step behind `make db-migration` that re-attaches a new column's own `REFERENCES` clause — read from `schema.sql` — to sqldef's generated `ADD COLUMN` statement before the allow-list and drift checks run on the candidate output, rather than adopting a second generator. This is tracked as its own follow-up `safety-critical` issue and **is not built yet**: today, `internal/store/tools/dbmigration` runs the same drift and allow-list checks `make db-check` would, on sqldef's own unmodified candidate output, *before* writing anything — so this case is still refused immediately (nothing is written or checksummed) exactly as described above, rather than only caught later by a separate `make db-check` run against an already-"immutable" file. "Another generator" is revisited only if a second, independent generator gap blocks real work — sqldef otherwise handles every ordinary case (add column, create table, whole-table drop) correctly and unconditionally.
