...
**Decided by the maintainer (2026-09-17):** an `INSERT INTO t SELECT ... ON CONFLICT DO UPDATE` inside a contract step still classifies as `classContractOnly` today (its leading shape is `INSERT ... SELECT`), which makes it effectively an `UPDATE` on every conflicting row despite the previous paragraph's "never UPDATE" — `classifyInsert` will be changed to refuse anything appearing after the `SELECT`, keeping a contract step to a plain copy and pushing any data change into a Go transform instead. This is tracked as its own follow-up `safety-critical` issue and **is not built yet** — today's classifier still accepts this shape as described above. Separately, an ordinary (non-contract) migration may create a `CREATE TRIGGER` whose body contains DML, which then fires on a later transform's or a contract step's own DML inside the same migration transaction, and again at ordinary runtime; a trigger with a DML body is decided to be **contract-only** (registered), also tracked as its own follow-up `safety-critical` issue and **not built yet** — today's classifier still allows it as an ordinary migration statement.
