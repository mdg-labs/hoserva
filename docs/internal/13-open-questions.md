...
**Status:** Spike (S9) — **loop devices, FUSE and a hosted SnapRAID sync confirmed on the pinned hosted runner image** (issue #10, doc 08 §9 "Hosted CI runners"); **AppArmor necessity still open** — the one run that tried it produced a void result, not a real one; **`/dev/kvm` still not confirmed** — a triggered run (`35049304081`) failed before ever invoking QEMU, at an unreadable-kernel bug that is now understood, fixed, and validated locally (not yet against a real hosted `/dev/kvm`) · **Gate:** Phase 1 · **Affects:** doc 06 §7, doc 07 R11
...
**Default:** everything that executes pull-request code runs on GitHub-hosted runners: L1, L2 loop devices via the runner's `sudo`, `.deb` build, and L3 if hosted KVM proves sufficient. There are no self-hosted runners (Q79). Workflows from first-time contributors require approval (a repository setting). **What's actually confirmed so far, not assumed:** `ci.yml`'s own `lab` job has run loop devices, XFS and a mergerfs pool mount successfully on a hosted runner in production (run 34950031773, 2026-09-15, on `ubuntu-latest`). The maintainer then triggered `s9-hosted-probe.yml` once (run 35049304081, `workflow_dispatch`, 2026-09-16): its `lab` job succeeded **on the pinned `ubuntu-24.04`**, closing that reproducibility gap, and ran a SnapRAID sync to completion on a hosted runner for the first time (`Everything OK`) — L2 plus a real parity sync both work hosted, on the pinned image. That same run's AppArmor with/without comparison is **void, not negative**: both branches failed identically from a probe-recipe bug (missing `mknod` of the container's own loop nodes) unrelated to AppArmor, yet the step still printed a "required" verdict — retracted in doc 08 §9, not carried forward; the fixed step has not yet run hosted. `/dev/kvm` support — and therefore whether L3 can move to hosted runners at all — remains untested: that run's `kvm` job failed at an unreadable-kernel check before `qemu-system-x86_64` was ever invoked (the runner's own real kernel image exists but is `0600 root:root`), though it did confirm the udev-rule mechanism itself works (`crw-rw----` → `crw-rw-rw-`) and that the CPU exposes `svm`. The kernel-read bug is now fixed (copy the running kernel out with the runner's own job-scoped `sudo`, `chown` the copy) and validated under the exact non-root privilege boundary the real script uses (doc 08 §9), but not yet against a real hosted `/dev/kvm` — that needs another maintainer-triggered run. Until that run exists, L3 stays on the dev host per Q79's own fallback.
...
**Status:** Default — **S9's lab half (loop devices, FUSE, a SnapRAID sync) is confirmed hosted on the pinned image** (issue #10, doc 08 §9); **S9's AppArmor-necessity question is still open (the one run that tried it produced a void result), S9's KVM half is still open (a triggered run failed before invoking QEMU — the bug is fixed and validated locally, not yet hosted), and S10 is entirely untouched** · **Gate:** Phase 1 · **Affects:** doc 06 §4, §7, doc 14 §8, Q42, D20
