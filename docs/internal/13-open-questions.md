...
**Status:** Default — **sqldef's SQLite output confirmed against sqlite3def v1.0.7; its blind spots — including on `CHECK`, `UNIQUE`, foreign-key actions, `COLLATE`, `STRICT`/`WITHOUT ROWID`, generated columns and a new column's own `REFERENCES` clause, not just column type — are why `make db-check`'s drift check is a tokenizer-based comparison of every stored object's whole canonicalized text, never a trust in sqldef's flags. Data safety is an allow-list, not a deny-list: every migration statement must be one of the shapes the generator emits, or, in a registered contract step, one of the documented rebuild shapes; anything else — including any spelling of `PRAGMA`, `EXPLAIN`, a `TEMP` object, transaction control, or DML beyond a contract step's own `INSERT ... SELECT` copy — is refused outright** · **Gate:** Phase 1 (foundation) · **Affects:** doc 00 D16, doc 01 §4, doc 06 §2, doc 12 §2, §3
...
Closing the `REFERENCES`/parser-rejection gaps above — another generator behind the same target, or a Hoserva post-processing step — remains open, for the maintainer to decide when it matters: an alternative SQLite-diffing tool was considered (see the tool comparison above) and not adopted so far, because sqldef is retained for the one thing it does correctly and unconditionally (add column, create table, whole-table drop), and every safety and drift property D16 needs is Hoserva's own code, not a flag from either tool. `github.com/sqldef/sqldef/cmd/sqlite3def`'s standalone binary is used only to run the confirmations above, as a manual, one-off check — nothing in the Makefile calls it; `internal/store/tools/dbmigration` uses sqldef as a library (`github.com/sqldef/sqldef/schema`) instead, which is the only sqldef dependency any build actually needs.

**Open maintainer questions this issue's own classifier surfaces, not yet decided:** an `INSERT INTO t SELECT ... ON CONFLICT DO UPDATE` inside a contract step still classifies as `classContractOnly` (its leading shape is `INSERT ... SELECT`), which makes it effectively an `UPDATE` on every conflicting row despite the previous paragraph's "never UPDATE" — whether `classifyInsert` should refuse anything appearing after the `SELECT` is open. Separately, an ordinary (non-contract) migration may create a `CREATE TRIGGER` whose body contains DML, which then fires on a later transform's or a contract step's own DML inside the same migration transaction, and again at ordinary runtime — whether a trigger with a DML body should itself be contract-only is also open.
...
**Open question for the maintainer:** `CREATE VIRTUAL TABLE` (for example, FTS5) is refused in both `schema.sql` and any migration — it matches none of `classifyStatement`'s recognized `CREATE TABLE`/`INDEX`/`VIEW`/`TRIGGER` shapes, so it falls to the disallowed default. This is consistent with the allow-list's fail-closed design, but a future search feature will need a decision: teach the classifier and drift check `CREATE VIRTUAL TABLE`'s own DDL shapes, or keep full-text search outside the SQLite-managed schema.
...
| The `/api/v1/events` SSE endpoint's server implementation (future work — no issue implements it yet) | Planned to be hand-written in `internal/api`, against the schemas `openapi.yaml` declares for it, once the job system and its live progress land (doc 01 §4) |
...
The `/api/v1/events` server write loop itself — framing an `Event` over `text/event-stream` — is still not generated by any tool checked here, and is not implemented in `internal/api` by this issue; that is unchanged, and is future work for the issue that wires the job system to live progress (doc 01 §4). No tool here generates that half from a schema — it is business logic (when to write, when to flush, how to end the connection) regardless of generator.
