...
**Status:** Default — **sqldef's SQLite output confirmed against sqlite3def v1.0.7; its blind spots — including on `CHECK`, `UNIQUE`, foreign-key actions, `COLLATE`, `STRICT`/`WITHOUT ROWID`, generated columns and a new column's own `REFERENCES` clause, not just column type — are why `make db-check`'s drift check is a tokenizer-based comparison of every stored object's whole canonicalized text, never a trust in sqldef's flags. Data safety is an allow-list, not a deny-list: every migration statement must be one of the shapes the generator emits, or, in a registered contract step, one of the documented rebuild shapes; anything else — including any spelling of `PRAGMA`, `EXPLAIN`, a `TEMP` object, transaction control, or DML beyond a contract step's own `INSERT ... SELECT` copy — is refused outright. The four generation/allow-list gaps this issue surfaced (`ADD COLUMN ... REFERENCES`, a contract step's own `ON CONFLICT DO UPDATE`, a trigger with a DML body, `CREATE VIRTUAL TABLE`) were decided by the maintainer on 2026-09-17 — see each paragraph below. The `REFERENCES` post-processing step (#133), the `ON CONFLICT` refusal (#134) and the DML-trigger contract-only classification (#135) have all landed** · **Gate:** Phase 1 (foundation) · **Affects:** doc 00 D16, doc 01 §4, doc 06 §2, doc 12 §2, §3

**Default:**

| Job | Tool |
|---|---|
| Generate the next schema migration from `schema.sql` | sqldef's SQLite generator (MIT), used as a Go library (`github.com/sqldef/sqldef/schema`, `.../database/sqlite3`) by `internal/store/tools/dbmigration`, comparing the schema the existing migrations produce with `schema.sql`, never with `--enable-drop-table` |
| Checksum, drift and data-safety checks | `make db-check` (`internal/store/tools/dbcheck`), Hoserva's own Go code — deliberately not sqldef's own diff (see findings below) |
| Typed queries | sqlc (MIT), `engine: sqlite`, reading `schema.sql`, run via `make gen` |
| Applying migrations on the user's machine | The runner in `internal/store` (package `store`), migrations embedded via `go:embed` — no migration tool ships in the `.deb` |

sqldef skips destructive changes unless `--enable-drop-table` is passed, which is D16's rule as a tool default, and it compares SQL files without needing a live database. Atlas was the other candidate, but its `migrate lint` — the part that detects destructive changes — requires an Atlas Pro login since v0.38, so it can't be this project's safety check.

**Confirmed against `github.com/sqldef/sqldef` v1.0.7's `cmd/sqlite3def`, run both as the standalone binary (`go run .../cmd/sqlite3def --dry-run <db> < desired.sql`) and as the library `sqlite3def.go` wraps (`schema.GenerateIdempotentDDLs`, which `internal/store/tools/dbmigration` calls directly):**

- **Added column** — clean: `sqlite3def --dry-run` against a table with an extra column in the desired schema emits exactly `ALTER TABLE `t` ADD COLUMN `c` <type> ...;`. This is the common case and needs nothing further.
- **New table** — the generator's "table not found" branch emits the desired `CREATE TABLE` statement byte-for-byte from the input, unmodified — so a fresh install's first migration is textually identical to `schema.sql`'s own `CREATE TABLE`.
- **`DROP TABLE` without `--enable-drop-table`** — skipped, printed as `-- Skipped: DROP TABLE ...;`, exactly as documented.
- **`DROP COLUMN`, with or without `--enable-drop-table`** — **emitted unconditionally.** Reading `database.RunDDLs`/`showDDLs` in sqldef v1.0.7 shows the gate is a bare `strings.Contains(ddl, "DROP TABLE")` on each statement; `DROP COLUMN` never matches that substring and is never gated. **The flag's name is misleading: it only ever gates whole-table drops.** This is the concrete reason D16's own data-safety scan (`make db-check`) cannot delegate to sqldef's flag and has to pattern-match migration content itself.
- **Column type change, `NOT NULL` change, `CHECK`/`UNIQUE` constraint change, on an existing column** — **not detected at all.** `sqlite3def --dry-run` reports `-- Nothing is modified --` for every one of these tried (`TEXT`→`INTEGER`, dropping `NOT NULL`, adding a table-level `CHECK`, adding `UNIQUE`, changing a `DEFAULT`). Reading `schema/generator.go`'s per-mode switch in `generateDDLsForCreateTable` confirms there is no `GeneratorModeSQLite3` case for "column exists but changed" at all — only MySQL, PostgreSQL and MSSQL have one. **sqldef's SQLite generator has no rebuild strategy** (no `CREATE TABLE ... _new_`, copy, drop, `RENAME TO` sequence) in this version, for any of these changes.
- **Column rename** — expressed as `ADD COLUMN <new>` followed by `DROP COLUMN <old>`, which is indistinguishable from "add a column, separately drop a different one" to a scanner reading generated SQL, and carries the same unconditional-emission gap as plain `DROP COLUMN` above.
- **A new column's own `REFERENCES` clause is silently dropped by sqldef itself** — confirmed directly against `schema.GenerateIdempotentDDLs`: a `schema.sql` change that adds `parent_id INTEGER REFERENCES parent(id)` to an existing table generates `ALTER TABLE `a` ADD COLUMN `parent_id` integer` with the foreign key gone entirely, not an error and not a skipped statement. `DEFAULT`, `CHECK` and `COLLATE` on a *new* column are all carried through correctly (confirmed by the generate-then-drift-check tests in `internal/store/tools/dbmigration/main_test.go`) — this is specific to `REFERENCES`. `internal/store/tools/dbmigration` closes this gap itself (#133), after generation and before the allow-list and drift checks run on the candidate output: `store.SchemaColumnReferences` reads every column's own inline `REFERENCES` clause out of `schema.sql`, using the same tokenizer the rest of this package's checks use, and `restoreAddColumnReferences` splices the clause found for a generated statement's own `(table, column)` — read with `store.AddColumnTarget` — back onto it. A column whose `REFERENCES` clause pairs with a `DEFAULT` other than `NULL` is refused outright instead of restored: SQLite itself refuses `ALTER TABLE ... ADD COLUMN ... REFERENCES ...` once foreign key constraints are enabled at prepare time unless the new column's own default is `NULL`, so restoring the clause onto that shape would only trade sqldef's silent drop for a statement SQLite itself can refuse to apply — this tool reports the specific reason instead. The candidate is still run through the same drift and allow-list checks `make db-check` would afterwards, so a mistake in this splicing is caught before anything is written, exactly as for every other generated migration. Adopting a second generator was not needed for this — sqldef otherwise handles every ordinary case (add column, create table, whole-table drop) correctly and unconditionally.
- **The parser itself rejects several spellings this schema could otherwise use** — confirmed directly against `sqldefdb.NewParser(sqldefparser.ParserModeSQLite3)`, each as its own `CREATE TABLE`/`CREATE TRIGGER` fed to `schema.GenerateIdempotentDDLs`: `NOT NULL ... COLLATE ...` (COLLATE must come *before* NOT NULL for this parser — `COLLATE NOCASE NOT NULL` parses; `NOT NULL COLLATE NOCASE` doesn't); any `ON CONFLICT` clause; `DEFAULT (datetime('now'))` (a parenthesized expression default calling a function); `rowid` referenced inside a trigger body; and an unquoted `begin` or `end` column name (both are ordinary SQLite identifiers, not reserved words, despite also being trigger-body keywords). Every one of these fails outright with a syntax error, for a brand-new table or trigger exactly as much as for a diff. `make db-migration` simply cannot be run at all against a `schema.sql` written with any of these spellings — a hard authoring constraint on `schema.sql`, not a drift-detection gap. **Also confirmed:** `DEFAULT (0)` — a parenthesized simple literal, needed nowhere SQLite doesn't also accept the bare form — round-trips through `ADD COLUMN` generation as `DEFAULT 0` (parens dropped); since the drift check compares token streams and parentheses are their own tokens, writing a new column's default with parentheses in schema.sql when it doesn't need any makes the drift check itself report a false-positive schema/migration mismatch (confirmed: `CheckDrift` fails schema.sql's `DEFAULT (0)` against a migration's generated `DEFAULT 0`). **Decided (maintainer, 2026-09-17):** none of this is worth a tooling change. These five parser-rejected spellings, and the `DEFAULT (0)` round-trip, become documented `schema.sql` authoring rules instead: write `COLLATE ... NOT NULL` in that order (never `NOT NULL ... COLLATE`); never write `DEFAULT (datetime('now'))` — use `CURRENT_TIMESTAMP`; never write `ON CONFLICT` in DDL; never reference `rowid` inside a trigger body; quote `begin`/`end` wherever either is a column name (see also `Key`/`value`/`status` below); and never parenthesize a `DEFAULT` literal that doesn't need parentheses. "Another generator" is revisited only if a second, independent gap blocks real work — see the Status line above.

**What this does, and doesn't, change about the design:** Hoserva's own D16 rule already requires a column type or constraint change to go through expand (add the new column) → transform (copy, converting) → contract (drop the old column, as a registered contract step) rather than an in-place `ALTER COLUMN`. sqldef's inability to detect or rebuild for such a change in place forces exactly that path already — there is no in-place SQLite rebuild statement for `make db-migration` to silently reach for instead, so this part needed no further decision. The `REFERENCES`-on-`ADD COLUMN` gap and the parser's rejected spellings above are different: they are open limitations of the tooling this issue confirmed but did not resolve, tracked above rather than worked around by prescribing a hand-written migration for cases D16 doesn't actually require one for.

The one real generation-independent gap is **drift detection**, and `internal/store/sqltoken.go`'s tokenizer-based, whole-statement comparison is what catches every clause a clause-by-clause approach would have to enumerate one at a time to catch at all — `ON CONFLICT`, `AUTOINCREMENT`, a partial-index `WHERE` clause, an expression-index column, a second `CHECK` on the same column, a `COLLATE` inside a `CHECK` expression, a `DEFERRABLE` clause, a comment containing a comma or an apostrophe, a `DEFAULT` written as `current_timestamp` instead of `CURRENT_TIMESTAMP`. `make db-check`'s schema-drift check (replaying every migration and comparing to `schema.sql`) does *not* ask sqldef whether the two match, and does *not* compare `sqlite_master.sql`'s raw stored text either — `ALTER TABLE ... ADD COLUMN` rewrites that text (quoting style, keyword case) on every column it ever adds, so a raw-text comparison would flag the generator's own normal output as drift. Instead, `internal/store/sqltoken.go` tokenizes every stored `CREATE TABLE`/`CREATE INDEX`/`CREATE TRIGGER`/`CREATE VIEW` statement into a canonical stream — comments and whitespace dropped, every keyword and unquoted or backtick-/bracket-quoted identifier ASCII-case-folded, every string literal kept exact — and `internal/store/schemastruct.go` compares that whole canonical stream, per table/index/trigger/view name, between a database built fresh from `schema.sql` and one built by replaying every migration. Because the comparison is the statement's *entire* text rather than an enumerated list of clauses read out of it, every one of the cases above — and `CHECK` (all of them), `COLLATE`, `ON CONFLICT`, `DEFERRABLE`, `AUTOINCREMENT`, a generated column's expression, `DEFAULT`, `NOT NULL`, a partial or expression index, a trigger's or view's own body — is caught without needing its own dedicated rule in the tokenizer. This is not a claim that the tokenizer has no gaps of its own — see the identifier-character and unrecognized-`ALTER TABLE`-shape limitations below.

Two deliberate exceptions to that folding: a double-quoted token's exact case is kept, never folded, because SQLite treats `"..."` as an identifier only if one by that name exists and otherwise falls back to a string literal — this tokenizer has no schema to resolve that against — and the single bare word immediately following a `DEFAULT` keyword is kept exact for the same reason (SQLite accepts an unquoted word there and treats it as a string, not an identifier). For this one narrow ambiguity, both exceptions are biased toward a false positive (two identical schemas reported as drift) rather than a false negative; that bias says nothing about `canonicalize`'s own, separate handling of whether a quoted word ever compares equal to an unquoted one, covered below. `strings.ToLower` folds Unicode case generally (for example `"Ä"` onto `"ä"`), which is not what SQLite itself does (SQLite folds ASCII case only) — folding is ASCII-only throughout for this reason.

A CREATE TRIGGER body's own `BEGIN ... END` block, and a `CASE ... END` inside it, are tracked correctly: `BEGIN`/`END` are ordinary, unquoted SQLite identifiers, not reserved words — a column can be named either — so counting every bare occurrence of either word anywhere would be wrong in both directions (an unrelated `begin`-named column elsewhere would inflate the count and could hide a real top-level statement boundary; a trigger body's own `CASE ... END` would close the tracked block early and leave the trigger's real closing `END` looking unbalanced). The scan opens a tracked block only for a statement whose own leading words are `CREATE [TEMP|TEMPORARY] TRIGGER`, and pairs `CASE` with `END` inside it; nowhere else does either word mean anything to this code.

SQLite 3.25+ rewrites a trigger's or view's own stored body on `ALTER TABLE ... RENAME TO`/`RENAME COLUMN` when it references the renamed object, the same way `ADD COLUMN` rewrites a table's own stored text; the tokenizer-based comparison handles this the same way (compare the canonicalized *result*, not the literal text). This check compares canonicalized *text*, not structure read PRAGMA by PRAGMA, which is what lets it cover a clause without a dedicated rule for that clause.

A quoted token's content can itself contain spaces or characters that read like other tokens (`"id integer"` is one column literally named `id integer`), so `canonicalize` never joins a quoted word token's text bare — it is re-quoted with its own quote marks before joining. Joining a double-quoted token's content bare would make `CREATE TABLE t ("id integer" TEXT)` and `CREATE TABLE t (id integer TEXT)` — two schemas SQLite treats as genuinely different — canonicalize to the identical string (`TestCheckDrift_DetectsQuotedIdentifierNotMergingWithBareWords`). Quoting every *quoted* word token this same way, regardless of which quote character the source used, is also what makes a schema.sql column declared `"key" TEXT` compare equal to sqldef's own generated `ADD COLUMN` output for it, which is always backtick-quoted and lower-cased (`` `key` text ``) — both become the word "key", quoted alike, so the two only ever differ when their folded content actually differs (`TestCheckDrift_NoFalsePositiveForDoubleQuotedVsBacktickIdentifier`, `internal/store/tools/dbmigration`'s own `TestRun_GeneratedAddColumnWithDoubleQuotedKey_NoDrift`).

An *unquoted* word, by contrast, is joined with no quoting of its own — with one exception: a bare word whose folded text is one of SQLite's own literal-value keywords (`NULL`, `TRUE`, `FALSE`, `CURRENT_TIME`, `CURRENT_DATE`, `CURRENT_TIMESTAMP`) renders with its own distinct marker instead, so it never compares equal to a quoted token spelling the same text. SQLite gives `DEFAULT NULL`'s bare `NULL` keyword and `DEFAULT "null"`'s quoted string/identifier fallback two genuinely different values — the same is true of `TRUE`/`FALSE`/`CURRENT_*`, wherever they appear bare, not only after `DEFAULT` (a partial index's own `WHERE ... IS NULL`, or a `CHECK` using `current_date`, for example) — and `canonicalize` must keep the two apart rather than reporting no drift between them (`TestCheckDrift_DetectsBareDefaultNullVsQuotedNullString` and the sibling tests next to it). Every other bare word (an ordinary identifier, a type name, a structural keyword like `CREATE` or `PRIMARY`) keeps comparing equal to its quoted form, which is exactly the equivalence the previous paragraph depends on.

**Two identifier-character rules the tokenizer must match:** `$` and any UTF-8 byte ≥ 0x80 are identifier characters to SQLite at every position, not just ASCII letters/digits/underscore — otherwise a table or column named `t$1` or `tä` would split into an identifier plus leftover characters read as something else, and a trigger-body identifier like `écase` (a bare word that merely ends in the ASCII letters "case") would scan byte-by-byte outside any identifier, exposing "case" as if it were the CASE keyword and mispairing a trigger's own `BEGIN...END` depth counter — which lets a real `COMMIT` after the trigger run uncaught by `Runner.Apply`, applying part of an otherwise-failing migration batch. The tokenizer's own `isIdentChar`/`isIdentStart` match SQLite's character classes for exactly this reason. The `DEFAULT`-bareword exception (a bare, unquoted word right after `DEFAULT` keeps its exact case) must fire only after `DEFAULT` was itself written unquoted — firing it after a *quoted* `"default"` too, because its folded content happens to read "default", would wrongly keep an unrelated word's case unfolded.

**A parser keyword gap not closed:** an unquoted column named `Key`, `value` or `status` — all ordinary SQL-92 reserved words sqldef's own parser rejects with a syntax error even though SQLite itself accepts each unquoted — has the same effect as the parser's other rejected spellings above: `make db-migration` cannot be run at all against a `schema.sql` using any of them. Quoting the name works around it (confirmed for `Key`; `value` and `status` behave identically) and is the decided authoring rule (maintainer, 2026-09-17), the same as the parser-rejected spellings above. `status` is worth naming explicitly here because a jobs table (#19) is likely to want a column by that name.

**The allow-list answers most of the DML question, and narrows what's left of it:** `internal/store/allowlist.go`'s classifier refuses DML outright in every migration, registered as a contract step or not — the only DML shape any migration may ever contain is a registered contract step's own `INSERT INTO <name> [(cols)] SELECT ...`, the documented rebuild sequence's copy step; a bare `DELETE`, `UPDATE`, `INSERT ... VALUES`, or any other DML shape is refused before it is ever applied. `internal/store/safety.go`'s `Destructive`/`CheckSafety` is a reporting layer over this same classifier — naming which contract-only shape a migration contains, for the audit trail — not an independent check standing between a hand-written contract step and an unreviewed `DELETE`/`UPDATE`. What's left for the maintainer to decide: whether a contract step's `INSERT ... SELECT` should remain the *only* DML a migration may ever carry, or whether some other narrow, reviewable shape should join it; see also the two questions below on a contract step's own `INSERT ... SELECT ... ON CONFLICT` and on triggers with a DML body.

The `REFERENCES` gap is fixed by the post-processing step above; the parser-rejection gaps are decided as `schema.sql` authoring rules rather than a tooling change (maintainer, 2026-09-17). An alternative SQLite-diffing tool was considered (see the tool comparison above) and not adopted, because sqldef is retained for the one thing it does correctly and unconditionally (add column, create table, whole-table drop), and every safety and drift property D16 needs is Hoserva's own code, not a flag from either tool. `github.com/sqldef/sqldef/cmd/sqlite3def`'s standalone binary is used only to run the confirmations above, as a manual, one-off check — nothing in the Makefile calls it; `internal/store/tools/dbmigration` uses sqldef as a library (`github.com/sqldef/sqldef/schema`) instead, which is the only sqldef dependency any build actually needs.

**Decided by the maintainer (2026-09-17):** an `INSERT INTO t SELECT ... ON CONFLICT DO UPDATE` inside a contract step used to classify as `classContractOnly` (its leading shape is `INSERT ... SELECT`), which made it effectively an `UPDATE` on every conflicting row despite the previous paragraph's "never UPDATE" — `classifyInsert` (#134) now refuses any `INSERT ... SELECT` whose statement carries an `ON CONFLICT` clause at paren depth 0 after the `SELECT`, keeping a contract step to a plain copy and pushing any data change into a Go transform instead; an `ON CONFLICT` that can only ever appear as string data nested inside the select itself is not mistaken for one. Separately, a `CREATE TRIGGER` whose body contains DML fires on a later transform's or a contract step's own DML inside the same migration transaction, and again at ordinary runtime, which makes it a data change, not merely a schema change; a trigger with a DML body is **contract-only** (registered), like the rebuild shapes above (#135). `classifyCreateTrigger` locates only the one part of the statement legitimately full of the words `INSERT`/`UPDATE`/`DELETE` — the event clause itself, `[BEFORE|AFTER|INSTEAD OF] {DELETE|INSERT|UPDATE [OF col, ...]} ON <table>` — structurally, then scans every remaining token of the statement (`FOR EACH ROW`, a `WHEN` expr, the `BEGIN...END` body, all of it) for an *unquoted* word spelling `insert`, `update`, `delete` or `replace`; finding one makes the whole trigger `classContractOnly`, and finding none leaves it `classOrdinary`. A quoted token (`"update"`, `` `delete` ``, `[replace]`, `'insert'`) never counts, in any of SQLite's four quoting styles, because the tokenizer's own `quoted` flag is checked directly rather than inferred from a keyword-depth count a quoted token could throw off — an earlier version of this classifier instead tried to locate the body's own `BEGIN` and split it into top-level statements the way `splitStatements` finds a trigger's own boundary, tracking a `begin`/`case`/`end` depth counter that did not check the `quoted` flag either; that version passed a DML-bodied trigger as `classOrdinary`, unregistered, whenever its DML target column was quoted (`SET "end" = 1`, `` SET `end` = 1``, `SET [case] = 1`) and whenever a `WHEN` clause without its own enclosing parentheses bare-referenced or quoted a column literally named `begin` (`WHEN NEW.begin > 0`, `WHEN begin`, `` WHEN NEW."begin" = 1``) ahead of the trigger's own real `BEGIN` — both fail-open gaps this scan-everything design removes by construction, since it never needs to find `BEGIN` or track any nesting depth at all. The one thing this design cannot tell apart from real DML is an unquoted `replace` used as an identifier (a column, table or alias name) or as the `REPLACE(...)` scalar function, inside an otherwise SELECT-only trigger — SQLite has no way to nest a real DML statement in any of those positions, so this is a false positive, never a false negative, and only ever costs a plain-SELECT trigger a contracts-file entry it doesn't strictly need, never a data-safety gap; a hand-written migration hitting it just registers the trigger's migration as a contract step. `CheckSchemaVocabulary` (which otherwise refuses every `classContractOnly` shape in `schema.sql`, since a DROP, a rebuild rename or a contract step's own `INSERT ... SELECT` are transitional steps that leave no trace in the desired end state) makes one narrow exception for exactly this shape: once a registered contract step creates a DML-bodied trigger, the trigger itself *is* part of the desired end state, not a transitional step, so `schema.sql` has to be able to declare it — otherwise `CheckDrift`'s replay-vs-`schema.sql` comparison could never agree once such a trigger exists at all. Every other contract-only shape is still refused in `schema.sql` exactly as before.

**A schema.sql authoring note:** sqldef's `ADD COLUMN` generator always appends the new column at the end of the existing column list, regardless of where schema.sql places it — a column inserted in the middle of an existing table's column list in schema.sql, rather than at the end, always reports as drift against the migration that actually added it (a real, permanent difference in column order, not a bug in the check). Add a new column to schema.sql at the end of its table's column list, not wherever it reads best.

**The allow-list itself:** every migration statement, ordinary or in a registered contract step, is classified by `internal/store/allowlist.go`'s `classifyStatement` — the same tokenizer and splitter the drift check uses — into exactly one of three outcomes: allowed in any migration (`CREATE TABLE`/`INDEX`/`VIEW`/`TRIGGER`, none of them `TEMP` or `temp.`-qualified, none of them `CREATE TABLE ... AS SELECT`; `ALTER TABLE ... ADD [COLUMN] ...`; `ALTER TABLE ... RENAME [COLUMN] ... TO ...`), allowed only in a registered contract step (`DROP TABLE`/`INDEX`/`VIEW`/`TRIGGER`; `ALTER TABLE ... DROP [COLUMN] ...`; `ALTER TABLE ... RENAME TO ...`; a contract step's own `INSERT ... SELECT`; a `CREATE TRIGGER` whose body contains DML), or refused outright (`EXPLAIN`, every `PRAGMA`, transaction control, `VACUUM`, `ATTACH`/`DETACH`, `REINDEX`, `ANALYZE`, any other DML, and any shape the classifier cannot fully parse — fail closed). `ALTER TABLE ... RENAME TO` is classified as a contract-only rebuild step, not an ordinary one: it is also the exact syntax SQLite's documented rebuild sequence uses for its own final, exchange-of-identity rename, and the classifier has no way to tell that shape apart from a plain, harmless table rename from the statement's text alone — treating every whole-table rename as contract-only is the safe direction for that ambiguity, and costs a real standalone rename nothing but a one-line contracts-file entry. `internal/store/safety.go`'s `Destructive`/`CheckSafety` stay as a reporting layer over this same classifier (which specific contract-only shape a migration contains, for the audit trail), not a second, independent check.

**Vetting runs before any tool ever executes a statement it hasn't checked, not after:** `make db-check` (`internal/store/tools/dbcheck`) and `make db-migration` (`internal/store/tools/dbmigration`) both replay migration SQL against a live SQLite connection — `CheckDrift`'s own comparison database, and `db-migration`'s own `currentSchemaDDLs` — and both vet the SQL first. `dbcheck.run` loads contracts and calls `CheckAllowedStatements`/`CheckSafety` before `CheckDrift`; `dbmigration.run` calls `CheckStatementVocabulary` on the existing migrations before `currentSchemaDDLs` replays them, and `CheckAllowedStatements` on its own newly generated migration before `CheckDrift` replays the candidate. `schema.sql` itself is executed verbatim by `CheckDrift` too, so `CheckDrift` calls `internal/store/allowlist.go`'s `CheckSchemaVocabulary` first — every statement in `schema.sql` must be `classOrdinary` (not even a registered contract-only shape belongs in the desired end state). This ordering keeps an `ATTACH`, `VACUUM INTO` or `PRAGMA` out of reach of every one of these tools: `TestCheckDrift_RefusesAttachInSchemaBeforeEverExecutingIt` and its `VACUUM INTO` sibling (`internal/store/drift_test.go`), and the equivalent migration-level tests in both tools' own test suites, assert refusal and that the target file — inside each test's own `t.TempDir()` — is never created. The shared `ATTACH` example (`internal/store/storetest.DisallowedStatementExamples`) is a real file path; `storetest.WithAttachTarget` rewrites it to a caller's own `t.TempDir()` wherever a test actually executes the examples against a live connection.

**The refusal is identifiable, not just non-nil:** `internal/store/allowlist.go` exports `ErrDisallowedStatement`, wrapped into every error `CheckAllowedStatements`, `CheckStatementVocabulary` and `CheckSchemaVocabulary` return for a `classDisallowed` statement, and asserted at both the Apply layer (`TestRunnerApply_RefusesEveryDisallowedStatementKind`) and the `db-check` layer (`TestRun_FailsOnEveryDisallowedStatementKind`), plus the schema-vocabulary layer (`TestCheckSchemaVocabulary_RefusesContractOnlyStatement`, `TestCheckSchemaVocabulary_RefusesDisallowedStatement`). Several of the refused kinds (`RELEASE`, `DETACH`, `BEGIN`, `VACUUM`, `COMMIT`, `ROLLBACK`, `INSERT ... VALUES`, `INSERT OR REPLACE ... SELECT`) would also fail if the classifier wrongly allowed them — SQLite itself rejects most of them on replay, for reasons unrelated to the classifier — so a test asserting only `err != nil` cannot tell a live classifier bug from that unrelated SQLite failure. Every such test asserts `errors.Is(err, store.ErrDisallowedStatement)` specifically, and `applyAndExpectNothingApplied` also asserts `Apply` returns an empty snapshot path, since the refusal happens before `Snapshot` is ever taken. `END`, `END TRANSACTION` and `REPLACE INTO` are entries in the shared example set (`internal/store/storetest.DisallowedStatementExamples`) alongside every other refused kind; each falls to the classifier's own default case.

**The shared disallowed-statement examples live in `internal/store/storetest`:** `DisallowedStatementExamples` exists only for tests — an ordinary `_test.go` file can't be imported by another package's tests (`internal/store/tools/dbcheck`'s) — so it lives in `internal/store/storetest`, a small, non-test package both import from their own `_test.go` files, keeping the data out of `internal/store`'s own production API. `storetest.WithAttachTarget` returns a copy of the map with the `ATTACH` example's target rewritten to a caller-supplied path, so a test that actually executes these examples against a live connection points the one example able to touch the filesystem at its own `t.TempDir()`, rather than the bare relative filename the map's own literal carries.

**Trigger and view bodies do not treat a double-quoted word as equal to the same bare word:** `canonicalize`'s quoted/bare equivalence (needed so a `schema.sql` column quoted one way compares equal to `ALTER TABLE ... ADD COLUMN`'s own re-splicing, quoted another way) does not hold inside a `CREATE TRIGGER` body or a `CREATE VIEW ... AS` body — SQLite does not resolve names there at creation time the way it resolves a column definition: a bare name matching no column fails only when the trigger fires or the view is read, while the same name double-quoted instead falls back to a string literal. `internal/store/schemastruct.go`'s `readSchemaObjects` canonicalizes a `trigger` or `view` object's stored text with a second function, `canonicalizeBody`, that never unifies a quoted word with a bare one — column definitions never appear inside either body (there is no `ALTER TRIGGER`/`VIEW ADD COLUMN`), so this never reopens the `ADD COLUMN` false positive the unified form exists to prevent for tables and indexes. `TestCheckDrift_DetectsBareVsQuotedWordInsideTriggerBody` pins this for a trigger's own `done` vs `"done"`. One edge this leaves for `schema.sql`'s own author: after `ALTER TABLE ... RENAME TO`, SQLite stores a view's or trigger's own reference to the renamed table double-quoted, so `schema.sql`'s view or trigger text must spell that reference double-quoted (`"u"`) to match — the bare spelling (`u`) reports as drift even though it names the same table. This is a false positive, not a data-loss risk.

**Decided (maintainer, 2026-09-17):** `CREATE VIRTUAL TABLE` (for example, FTS5) is refused in both `schema.sql` and any migration — it matches none of `classifyStatement`'s recognized `CREATE TABLE`/`INDEX`/`VIEW`/`TRIGGER` shapes, so it falls to the disallowed default — and **stays refused** until a search feature actually needs it; that feature's own issue is where the classifier and drift check gain `CREATE VIRTUAL TABLE`'s DDL shapes, not this one speculatively.
