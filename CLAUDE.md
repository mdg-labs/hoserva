...
- Loop devices are host-global: detach only the ones backed by your own lab's image files. **Never `losetup -D`.** To confirm your own teardown left nothing attached, run `find /sys/devices/virtual/block -maxdepth 3 -path '*/loop/backing_file' -exec cat {} +` — never `losetup -a`/`-l` on the host for this. `/sys/block/loopN` is a *symlink* into `/sys/devices/virtual/block/loopN`, and plain `find` does not follow symlinks, so the check has to walk the real path, not `/sys/block` itself. Every entry there exists whether or not a loop device is bound; only the `loop/` subdirectory inside it appears once a backing file is attached, so this command prints one backing-file path per attached device and, on a clean teardown, prints **nothing and exits 0**. Do not add `-L` to walk `/sys/block` instead — `/sys/block/loopN/subsystem` symlinks back to `/sys/block` itself, so `-L` reports a filesystem loop and exits non-zero even when clean. A shell glob (`/sys/block/loop*/loop/backing_file`) is not this check either — it also errors when nothing matches.
...
- **Issues are the only plan.** Every phase in doc 07 §1 is an epic with sub-issues and a milestone. Doc 07 keeps the reasoning; status, scope and discussion live in the issues. There is no roadmap file to keep in sync.
- **New work starts as an issue** via `github-triage` — including a new phase's epic and its sub-issues.
...
- **`status:*` is machine-managed — exactly one per issue, never set by hand.** `.github/workflows/issue-status.yml` owns opened/reopened → `status:new` and closed → `status:closed`/`status:cancelled`; everything between goes through `scripts/issue-status.sh <issue> <status>`. `github-triage` sets `ready`; `orchestrate`'s executor and verifier own `in-progress` → `in-review` → `implemented`. An epic's status is rolled up by `scripts/epic-status.sh <epic>`. Never `gh issue edit --add-label status:…`. **Never read a `status:*` label back to confirm your own change** — the workflow owns those transitions and may not have run yet, so a read-back taken moments later reports a stale value, not a problem. Call the script (or `gh issue close`) and move on; nobody needs to supervise the status workflow.
